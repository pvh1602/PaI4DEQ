...
class MaskedMONSingleFc(nn.Module):
    ...
    def reinit(self):
        print('Initializing mask all entries which are not in the diagonal of A ...')
        # nn.init.constant_(self.A.weight, 1)
        # nn.init.constant_(self.B.weight, 1)
        # nn.init.constant_(self.U.weight, 1)
        mask_non_diagonal = torch.ones_like(self.A.weight) - torch.eye(self.A.weight.shape[0])
        self.A.weight_mask.data.copy_(mask_non_diagonal)
...
class MONSigmoid(nn.Module):
    def forward(self, *z):
        return tuple(F.sigmoid(z_) for z_ in z)

    def derivative(self, *z):
        return tuple(F.sigmoid(z_)*(1-F.sigmoid(z_)) for z_ in z)

class MONTanh(nn.Module):
    def forward(self, *z):
        return tuple(F.tanh(z_) for z_ in z)

    def derivative(self, *z):
        return tuple((1 - F.tanh(z_)**2) for z_ in z)
...
class MONLeakyReLU(nn.Module):
    ...
    def derivative(self, *z):
        # print(z)
        # print('Updated 2')
        # z = torch.where(z>0, 1, 0.01)
        return tuple(torch.where(z_>0, 1, 0.01).type_as(z[0]) for z_ in z)
        
        return tuple(z_.type_as(z[0]) for z_ in z)
        
        der_z = []
        for z_ in z:
            der_z.append(torch.tensor([1 if (z__>0) else 0.01 for z__ in z_]).type_as(z[0]))
        return tuple(der_z)
                
        return tuple(torch.ones_like(z_).type_as(z[0]) if (z_>0) else torch.ones_like(z_)*0.01.type_as(z[0]) for z_ in z)
        der_z = []
        for z_ in z:
            print(z_)
            if z_ > 0:
                der_z.append(torch.tensor(1.).type_as(z_))
            else:
                der_z.append(torch.tensor(0.01).type_as(z_))
        return tuple(der_z)
        # return tuple((z_ > 0).ty for z_ in z)
...
class MaskedMONSingleConv(nn.Module):
    """ MON class with a single 3x3 (circular) convolution """

    def __init__(self, in_channels, out_channels, shp, kernel_size=3, m=1.0):
        super().__init__()
        self.U = Conv2d(in_channels, out_channels, kernel_size)
        self.A = Conv2d(out_channels, out_channels, kernel_size, bias=False)
        self.g = nn.Parameter(torch.tensor(1.))
        self.h = nn.Parameter(torch.tensor(1.))
        self.B = Conv2d(out_channels, out_channels, kernel_size, bias=False)
        self.pad = 4 * ((kernel_size - 1) // 2,)
        self.shp = shp
        self.m = m
    ...
    def forward(self, x, *z):
        # circular padding is broken in PyTorch
        return (F.conv2d(self.cpad(x), self.U.get_masked_weight(), self.U.bias) + self.multiply(*z)[0],)
...
class MaskedMONBorderReLU(nn.Module):
    def __init__(self, border=1):
        super().__init__()
        self.border = border
...
class MaskedMONMultiConv(nn.Module):
    def __init__(self, in_channels, conv_channels, image_size, kernel_size=3, m=1.0):
        ...
        # create convolutional layers
        self.U = Conv2d(in_channels, conv_channels[0], kernel_size)
        self.A0 = nn.ModuleList([Conv2d(c, c, kernel_size, bias=False) for c in conv_channels])
        self.B0 = nn.ModuleList([Conv2d(c, c, kernel_size, bias=False) for c in conv_channels])
        self.A_n0 = nn.ModuleList([Conv2d(c1, c2, kernel_size, bias=False, stride=2)
                                   for c1, c2 in zip(conv_channels[:-1], conv_channels[1:])])
